...
from guniflask.security.preauth_provider import PreAuthenticatedProvider
from guniflask.security.user_details_by_name_service import UserDetailsByNameService
...
class AuthorizationServerEndpointsConfigurer:
    def __init__(self, ):
        ...
        self._default_token_services: DefaultTokenServices = None
    ...
    def set_client_details_service(self, client_details_service: ClientDetailsService):
        self._client_details_service = client_details_service

    def get_default_authorization_token_services(self) -> AuthorizationServerTokenServices:
        if self._default_token_services is None:
            self._default_token_services = self._create_default_token_services()
        return self._default_token_services
    ...
    @property
    def token_services_override(self) -> bool:
        return self._token_services_override

    @property
    def user_details_service_override(self) -> bool:
        return self._user_details_service_override
    ...
    def _get_client_details_service(self) -> ClientDetailsService:
        ...
        if self._default_token_services:
            self._add_user_details_service(self._default_token_services, self._user_details_service)
        return self._client_details_service

    def _add_user_details_service(self, token_services: DefaultTokenServices,
                                  user_details_service: UserDetailsService):
        if user_details_service:
            provider = PreAuthenticatedProvider(UserDetailsByNameService(user_details_service))
            token_services.authentication_manager = ProviderManager(providers=[provider])
...
class AuthorizationServerSecurityConfigurer(SecurityConfigurerAdapter):
    def __init__(self):
        super().__init__()
        self.password_encoder: PasswordEncoder = None

    def init(self, http: HttpSecurity):
        if self.password_encoder:
    ...
    def _get_password_encoder(self) -> PasswordEncoder:
        return NullablePasswordEncoder(self.password_encoder)
